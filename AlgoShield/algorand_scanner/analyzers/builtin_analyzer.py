...
class BuiltinAnalyzer(BaseAnalyzer):
    ...
    async def _analyze_python(self, file_path: Path, parsed_content: Dict[str, Any]) -> List[Vulnerability]:
        ...
        vulnerabilities.extend(self._check_hardcoded_credentials_python(file_path, lines))
        
        # Deduplicate vulnerabilities before returning
    ...
    def _check_missing_access_control_python(self, file_path: Path, lines: List[str], parsed_content: Dict[str, Any]) -> List[Vulnerability]:
        ...
        # Look for critical functions that should have access control
        critical_function_patterns = [
            r'def\s+delete_contract\s*\(',
            r'def\s+admin_',
            r'def\s+withdraw',
            r'def\s+transfer',
            r'OnComplete\.DeleteApplication'
        ]
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Check if we're entering a critical function
            for pattern in critical_function_patterns:
                if re.search(pattern, line):
                    in_function = True
                    function_match = re.search(r'def\s+(\w+)', line)
                    function_name = function_match.group(1) if function_match else "critical_function"
                    function_start_line = i
                    break
            
            # If we're in a critical function, look for access control
            if in_function:
                ...
                access_control_patterns = [
                    ...
                    r'Assert\(',
                    r'If\(.+sender',
                    r'Global\.creator_address',
                    r'check_admin',
                    r'is_admin'
                ]
                
                has_access_control = any(re.search(pattern, line, re.IGNORECASE) for pattern in access_control_patterns)
                
                # If we find a return without access control
                if (re.search(r'return\s+Approve\(\)', line) or 
                    re.search(r'return\s+Reject\(\)', line)) and not has_access_control:
                    ...
                    vulnerabilities.append(self._create_vulnerability(
                        ...
                        message=f"Critical function '{function_name}' lacks access control",
                        description="Critical functions should implement proper access control to prevent unauthorized access",
                        cwe_id="CWE-284",
                        code_snippet=lines[function_start_line-1].strip() if function_start_line > 0 else '',
                        fix_suggestion="Add sender verification or admin checks before allowing critical operations"
                    ))
                    in_function = False
                
                # Reset if we exit the function (new function or class definition)
                if line.strip() and not line.startswith(' ') and not line.startswith('\t') and re.search(r'^(def|class)', line):
                    in_function = False
        
        # Also check contracts from parsed content
    ...
    def _check_arithmetic_vulnerabilities_python(self, file_path: Path, lines: List[str]) -> List[Vulnerability]:
        ...
        # More specific patterns for PyTeal arithmetic operations
        arithmetic_patterns = [
            r'Add\s*\(',
            r'Sub\s*\(',
            r'Mul\s*\(',
        ...
        for i, line in enumerate(lines, 1):
            ...
            # Check for PyTeal arithmetic operations without bounds checking
            for pattern in arithmetic_patterns:
                if re.search(pattern, line):
                    # Check if this is actually an arithmetic operation, not just in a comment
                    if not re.search(r'#.*' + pattern, line):
                        ...
                        if not re.search(r'(Assert.*[<>]=?|require.*[<>]=?|If.*[<>]=?)', context, re.IGNORECASE):
                            vulnerabilities.append(self._create_vulnerability(
                                file_path=file_path,
                                line=i,
    ...
    def _deduplicate_vulnerabilities(self, vulnerabilities: List[Vulnerability]) -> List[Vulnerability]:
        """Remove duplicate vulnerabilities based on file, line, and rule_id"""
        seen = set()
        deduplicated = []
        ...
        if len(vulnerabilities) != len(deduplicated):
            print(f"Deduplicated {len(vulnerabilities)} vulnerabilities to {len(deduplicated)} in builtin analyzer")
        
        return deduplicated
    
    def _check_hardcoded_credentials_python(self, file_path: Path, lines: List[str]) -> List[Vulnerability]:
        """Check for hardcoded credentials and addresses"""
        vulnerabilities = []
        
        # Patterns for hardcoded credentials
        hardcoded_patterns = [
            r'Addr\s*\(\s*["\'][A-Z0-9]{58}["\']',  # Algorand address format
            r'hardcoded_admin\s*=',
            r'admin_address\s*=.*["\'][A-Z0-9]',
            r'private_key\s*=',
            r'secret\s*=',
            r'password\s*='
        ]
            ...
            for pattern in hardcoded_patterns:
                if re.search(pattern, line, re.IGNORECASE):
                    vulnerabilities.append(self._create_vulnerability(
                        ...
                        rule_id="py_hardcoded_credentials",
                        message="Hardcoded credentials or addresses detected",
                        description="Hardcoded credentials pose a security risk and should be externalized",
                        cwe_id="CWE-798",
                        code_snippet=line.strip(),
