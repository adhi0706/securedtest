...
const logo = "/assets/images/securedapp-logo-light.svg";
    ...
    try {
      ...
      const data = await fetch(apiUrl + "/payment-insert-web3", {
        ...
        body: JSON.stringify({
          ...
          couponCode,
          platform: "algoshield", // Add platform identifier
        }),
        headers: {
          "Content-type": "application/json",
          Authorization: getJwt(),
...
export const getScanSummaryData = async ({ dispatch, email }) => {
  ...
  if (history.length === 0) {
    // No scan history - set default values
    dispatch(
      setScanSummary({
        percentageValue: "10.0",
        values: [
          { name: "Critical Issues", value: 0, color: "#ff6666" },
          { name: "Medium Issues", value: 0, color: "#ffa366" },
          { name: "Low Issues", value: 0, color: "#ffff33" },
          { name: "Optimization Issues", value: 0, color: "#d98cb3" },
          { name: "Informational Issues", value: 0, color: "#b3b3b3" },
        ],
        summary: "No scans performed yet",
        id: 0,
      })
    );
    return;
  ...
  if (latestScan) {
    // For AlgoShield scans, we get data directly from localStorage
    // The scan data structure is different from the original SolidityShield
    const scanData = latestScan.scanResults;
    ...
    if (scanData && scanData.summary) {
      ...
      // Create summary text
      const summaryText = `Scanned ${summary.files_scanned || 1} file(s), found ${totalVulns} vulnerabilities`;
      
      // Map vulnerability counts to the expected format
      const data = {
        high_issues: summary.critical || 0,
        medium_issues: summary.high || 0,
        low_issues: summary.medium || 0,
        optimization_issues: 0,
        informational_issues: summary.low || 0
      };
      
      // Calculate score (5 - (total issues / 30) * 5)
      const totalIssues = data.high_issues + data.medium_issues + data.low_issues;
      const score = Math.max(0, 5 - (totalIssues / 30) * 5);
...
export const getIssuesChartData = async ({ dispatch, email }) => {
  ...
  if (history.length === 0) {
    // Return default data structure if no reports found
    const defaultData = [
      { name: "", value: 0 },
      { name: "High Issues", value: 0 },
      { name: "Medium Issues", value: 0 },
      { name: "Low Issues", value: 0 },
      { name: "Optimization Issues", value: 0 },
      { name: "Informational Issues", value: 0 },
      { name: "", value: 0 },
    ];
    dispatch(setIssuesData(defaultData));
    return defaultData;
  }
  ...
  for (let i = 0; i < history.length; i++) {
    const scanData = history[i].scanResults;
    if (scanData && scanData.summary) {
      const summary = scanData.summary;
      aggregatedData.high_issues += summary.critical || 0;
      aggregatedData.medium_issues += summary.high || 0;
      aggregatedData.low_issues += summary.medium || 0;
      aggregatedData.optimization_issues += 0; // AlgoShield doesn't have optimization issues
      aggregatedData.informational_issues += summary.low || 0;
    }
  }
  
  const chartData = [
    { name: "", value: 0 },
    { name: "High Issues", value: aggregatedData.high_issues },
    { name: "Medium Issues", value: aggregatedData.medium_issues },
    { name: "Low Issues", value: aggregatedData.low_issues },
    { name: "Optimization Issues", value: aggregatedData.optimization_issues },
    { name: "Informational Issues", value: aggregatedData.informational_issues },
    { name: "", value: 0 },
  ];
  
  dispatch(setIssuesData(chartData));
  return chartData;
};

export const getReport = async ({ id, email }) => {
  try {
    // For AlgoShield, get report data from localStorage
    const scanKey = `algoshield_scan_${id}`;
    ...
    const scanResults = parsedData.scanResults;
    
    if (!scanResults || !scanResults.findings) {
      console.error(`Invalid scan results for ID: ${id}`);
      return null;
    }
    
    // Calculate score based on findings
    const findings = scanResults.findings;
    const score = 5 - ((Number(findings.high_issues || 0) + 
                        Number(findings.medium_issues || 0) + 
                        Number(findings.low_issues || 0)) / 30) * 5;
    
    return {
      ...scanResults,
      score: (score * 2).toFixed(1) + "/10",
      findings: findings
    };
  } catch (error) {
    console.error("Error getting AlgoShield report:", error);
    return null;
  }
};
...
export const getScanHistoryData = async ({ userEmail, dispatch }) => {
  try {
    // Get all AlgoShield scan entries from localStorage
    const history = [];
    
    // Iterate through all localStorage keys to find AlgoShield scans
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('algoshield_scan_')) {
        try {
          const scanData = JSON.parse(localStorage.getItem(key));
          if (scanData && scanData.id) {
            // Format the data for the history table
            history.push({
              id: scanData.id,
              date: scanData.timestamp || new Date().toISOString(),
              status: 'Succeeded', // All stored scans are successful
              fileName: scanData.fileName || 'Unknown',
              companyName: scanData.companyName || 'Unknown',
    ...
    dispatch(setScanHistory(sortedHistory));
    
    return sortedHistory;
  } catch (error) {
    console.error("Error getting AlgoShield scan history:", error);
    return [];
  }
};
...
export const sendOTP = async ({ email, dispatch, selector }) => {
  ...
  try {
    ...
    const response = await fetch(apiUrl + "/sendOtp2", {
      method: "POST",
...
export const verifyOTP = async ({ email, otp, dispatch }) => {
  ...
  try {
    ...
    const response = await fetch(apiUrl + "/verifyOtp2", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    dispatch(
      login({
        ...
        credits: userdata.algoshield_credit || userdata.credit || 0,
        remainingCredits: userdata.algoshield_rcredit || userdata.rcredit || 0,
        plan: userdata.algoshield_plan || userdata.plan || 0,
        planName: plandetail,
        planExpiry: userdata.algoshield_planexpiry || userdata.planexpiry,
    ...
    typeof window !== "undefined" &&
      window.location.replace("/algoshield");
  } catch (error) {
    console.error("Error verifying OTP:", error);
    toast.error("Unable to verify OTP. Please try again!");
    typeof window !== "undefined" &&
      window.location.replace("/algoshield-scan/auth");
  }
};
...
export const downloadReport = async (id, user) => {
  try {
    ...
    if (!scanData) {
      toast.error("Scan report not found");
      return;
    }
    ...
    if (!scanResults) {
      toast.error("Invalid scan results");
      return;
    }
    
    // Check credits (for now, allow download for all users)
    ...
    console.log("Generating PDF for AlgoShield scan:", scanResults);
    generateAlgoShieldPDF(scanResults, parsedData);
  } catch (error) {
    console.error("Error downloading AlgoShield report:", error);
...
export const generateAlgoShieldPDF = async (scanResults, fullData) => {
  try {
    console.log("Starting AlgoShield PDF generation");
    ...
    const handleTextWrapping = (text, maxWidth, x, y) => {
      ...
      for (let i = 0; i < words.length; i++) {
        let testLine = line + words[i] + " ";
        let testWidth = pdf.getStringUnitWidth(testLine) * pdf.internal.getFontSize();
        if (testWidth > maxWidth) {
    ...
    // Set background color on the left side (green for AlgoShield)
    pdf.setFillColor(4, 170, 109);
    pdf.rect(0, 0, 50, pdf.internal.pageSize.getHeight(), "F");
    ...
    try {
      pdf.addImage(logo, "SVG", 89, 108, 15, 15);
    } catch (logoError) {
      console.warn("Could not add logo to PDF:", logoError);
      // Continue without logo
    }
    pdf.text("AlgoShield", 105, 120);
    pdf.line(50, 0, 50, 300);
    ...
    pdf.setFontSize(18);
    pdf.setFont("times", "bold");
    pdf.text("AlgoShield Audit Report", 43, 20);
    ...
    const headers = [
      ["SCAN_ID", fullData.id || "N/A"],
      ["Files Scanned", summary.files_scanned || 1],
      ["Total Vulnerabilities", summary.total_vulnerabilities || 0],
      ["Scan Duration", summary.scan_duration ? `${summary.scan_duration.toFixed(2)}s` : "N/A"],
      ["Tools Used", summary.tools_used ? summary.tools_used.join(", ") : "Builtin + Tealer"],
    ];
    ...
    // Findings Summary
    const findingsHeaders = [["Vulnerability Level", "Count"]];
    const findingsData = [
    ...
    // Add vulnerability details pages
    if (vulnerabilities && vulnerabilities.length > 0) {
      vulnerabilities.forEach((vuln, index) => {
        ...
        // Try to add logo, but don't fail if it doesn't work
        try {
          pdf.addImage(logo, "SVG", 10, 11, 10, 10);
        } catch (logoError) {
          console.warn("Could not add logo to PDF page:", logoError);
          // Continue without logo
        }
        pdf.setFontSize(13);
        pdf.setFont("times", "bold");
        pdf.text("AlgoShield", 21, 19);
        pdf.text(date, 170, 20);
        pdf.setDrawColor(0, 128, 0);
        pdf.line(10, linePositionY, 200, linePositionY);
        pdf.line(10, 270, 200, 270);
        ...
        pdf.setFontSize(18);
        pdf.setFont("times", "bold");
        pdf.text("Vulnerability Details", 75, 35);
        ...
        if (vuln.message) {
          pdf.setFontSize(16);
          pdf.setFont("times", "bold");
          pdf.text("Description", 15, pdf.lastAutoTable.finalY + 20);
          pdf.setFont("times", "normal");
          pdf.setFontSize(12);
          
          handleTextWrapping(vuln.message, 180, 15, pdf.lastAutoTable.finalY + 30);
        }
        ...
        if (vuln.code_snippet) {
          pdf.setFontSize(16);
          pdf.setFont("times", "bold");
          pdf.text("Code Snippet", 15, pdf.lastAutoTable.finalY + 50);
          pdf.setFont("times", "normal");
          pdf.setFontSize(10);
          pdf.text(vuln.code_snippet, 15, pdf.lastAutoTable.finalY + 60);
        }
        ...
        if (vuln.fix_suggestion) {
          pdf.setFontSize(16);
          pdf.setFont("times", "bold");
          pdf.text("Fix Suggestion", 15, pdf.lastAutoTable.finalY + 80);
          pdf.setFont("times", "normal");
          pdf.setFontSize(12);
          handleTextWrapping(vuln.fix_suggestion, 180, 15, pdf.lastAutoTable.finalY + 90);
        }
    ...
    const disclaimerData = [
      [
        "Purpose",
        "This AlgoShield audit report is provided for informational purposes only",
      ],
      [
        "Scope",
        "The audit was performed based on the state of the Algorand smart contract at the time of the audit and may not reflect its current state or any subsequent changes.",
      ],
      [
        "Limitations",
        "While every effort has been made to ensure the accuracy and completeness of this report, no guarantee is made that all vulnerabilities or issues have been identified. Security audits do not guarantee complete system security.",
      ...
      [
        "Recommendations",
        "The recommendations provided in this report are based on the best judgment of SecureDApp's security professionals. Implementation of these recommendations is at the discretion of the smart contract's maintainers.",
      ],
      [
        "Responsibility",
        "It remains the responsibility of the smart contract's maintainers and users to ensure its security and proper functionality. SecureDApp does not accept any liability for any damage or loss caused due to overlooked vulnerabilities or misinterpretations in this report.",
      ],
    ];
    ...
    try {
      ...
      console.warn("Could not add logo to PDF footer:", logoError);
      // Continue without logo
    }
    pdf.setFontSize(13);
    pdf.setFont("times", "bold");
    ...
    pdf.save("AlgoShield_Audit_Report.pdf");
    console.log("AlgoShield PDF generation completed");
    return pdf;
  } catch (error) {
    console.error("Error generating AlgoShield PDF:", error);
